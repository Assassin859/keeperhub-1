...
| `insufficient_scope` | 403 | The caller is not allowed the required scope | Use a key with the needed scope. For an agent connection, check whether an organization limit is holding it below its token's scope |
