...
- `pnpm test tests/integration/protocol-coverage/{slug}/<chain-name>/coverage.test.ts` PER CHAIN in `TEST_DATA`, gated on the chain's RPC env var (`SEPOLIA_RPC_URL`, `MAINNET_RPC_URL`, etc.). End-to-end execution via the shared coverage engine. Skipped is NOT pass. **Applies to Pattern A and C only.** Pattern B (mainnet-only, anvil fork) protocols omit `TEST_DATA` and have no coverage runner; the calldata `-onchain.test.ts` against the fork is their sole integration check (see Phase 1.7).
