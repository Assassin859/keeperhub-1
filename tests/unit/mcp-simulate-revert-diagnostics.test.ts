...
const SIMULATED_TARGET_RE = /Simulated target: 0xtoken/;
...
describe("MCP dry-run revert diagnostics", () => {
  ...
  it("keeps the machine-readable code when the simulator attributed one", async () => {
    mock400(
      JSON.stringify({
        ...(JSON.parse(REVERT_BODY) as Record<string, unknown>),
        code: "insufficient_balance",
        balanceWei: "1000",
        shortfallWei: "9000",
      })
    );
    await expect(invoke("execute_transfer")).rejects.toThrow(REASON_CODE_RE);
  });
